...
static int otrcp_get_tid_cmd_internal(const uint8_t *buf, size_t len, spinel_tid_t *ptid, uint32_t *pcmd)
{
	...
	*ptid = SPINEL_HEADER_GET_TID(header);
...
static int otrcp_spinel_send_receive(struct otrcp *rcp, struct sk_buff *skb, uint32_t cmd,
				     ...
				     postproc_func postproc, void *ctx, const char *fmt, ...)
{
	...
	if (rc >= 0) {
		rc = otrcp_spinel_send_receive_v(rcp, skb, postproc, ctx, fmt, args);
	}

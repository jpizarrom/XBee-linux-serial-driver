	...
	do {                                                                                       \
		...
		if (!skb) {                                                                        \
			return -ENOMEM;                                                            \
                                                                                                   ...
                                                                                                   \
		return otrcp_spinel_command(rcp, skb, cmd, arg, expected, postproc, ctx, fmt,      \
					    __VA_ARGS__);                                          \
	} while (0);
...
#define SPINEL_PROP_IMPL_V(prop, rcp, cmd, postproc, ctx, ...)                                     \
	do {                                                                                       \
		...
		SPINEL_IMPL_V(CONCATENATE(SPINEL_PROP_, prop), rcp, cmd, &expected, postproc, ctx, \
			      CONCATENATE(spinel_data_format_str_, prop), __VA_ARGS__)             \
	} while (0);

#define SPINEL_GET_PROP_IMPL(prop, rcp, ...)                                                       \
	SPINEL_PROP_IMPL_V(prop, rcp, SPINEL_CMD_PROP_VALUE_GET, postproc_unpack, NULL, __VA_ARGS__)

#define SPINEL_SET_PROP_IMPL(prop, rcp, ...)                                                       \
	SPINEL_PROP_IMPL_V(prop, rcp, SPINEL_CMD_PROP_VALUE_SET, postproc_return, NULL, __VA_ARGS__)
...
static const char *spinel_command_args_format[] = {
	NULL,			       // SPINEL_CMD_NOOP = 0
	SPINEL_DATATYPE_UINT8_S,       // SPINEL_CMD_RESET = 1
	SPINEL_DATATYPE_UINT_PACKED_S, // SPINEL_CMD_PROP_VALUE_GET = 2,
	SPINEL_DATATYPE_UINT_PACKED_S, // SPINEL_CMD_PROP_VALUE_SET = 3,
};
...
static int otrcp_format_command(uint8_t *buf, size_t len, uint32_t cmd, union spinel_cmdarg cmdarg,
			       spinel_tid_t tid, const char *format, va_list args)
{
	...
	if (cmd == SPINEL_CMD_RESET) {
		rc = spinel_datatype_pack(buf, len, spinel_command_args_format[cmd], cmdarg.reset);
	} else if (cmd == SPINEL_CMD_PROP_VALUE_SET) {
		rc = spinel_datatype_pack(buf, len, spinel_command_args_format[cmd], cmdarg.prop);
		if (rc < 0)
			return rc;
		...
		rc = spinel_datatype_vpack(buf, len, format, args);
...
static int otrcp_format_command_skb_v(struct otrcp *rcp, uint32_t cmd, union spinel_cmdarg cmdarg,
				      ...
				      va_list args)
{
	...
	send_buf = kmalloc(spinel_max_frame_size, GFP_KERNEL);
	if (!send_buf)
		return -ENOMEM;
	...
	rc = otrcp_format_command(send_buf, send_buflen, cmd, cmdarg, tid, fmt, args);
	if (rc < 0)
		goto exit;

	memcpy(skb_put(skb, rc), send_buf, rc);
				       ...
				       va_list args)
{
	...
	expected =
		*((struct otrcp_received_data_verify *)(skb->data + skb->len - sizeof(expected)));

	if ((rc = spinel_datatype_unpack(skb->data + expected.offset, skb->len - expected.offset,
					 "Ci", &header, &cmd)) < 0)
		return rc;
				...
				void *ctx, const char *fmt, ...)
{
	...
	rc = otrcp_format_command_skb_v(rcp, cmd, cmdarg, skb, postproc, ctx, expected, fmt, args);
	if (rc >= 0) {
...
static int otrcp_reset(struct otrcp *rcp, uint32_t reset)
{
	...
	return otrcp_spinel_command(rcp, skb, SPINEL_CMD_RESET, arg, &expected, postproc_return,
				    NULL, spinel_data_format_str_RESET,
...
static int otrcp_get_caps(struct otrcp *rcp, uint32_t *caps, size_t caps_len)
{
	...
	SPINEL_PROP_IMPL_V(CAPS, rcp, SPINEL_CMD_PROP_VALUE_GET, postproc_array_unpack_packed_int,
			   &datalen, caps, caps_len);
}
	...
	if (tid == 0) {
		...
		while ((skb = skb_dequeue(&rcp->xmit_queue)) != NULL) {
			...
			if (expected_key == SPINEL_PROP_STREAM_RAW) {
				...
				pr_debug("offset =%d\n", offset);
				pr_debug("tx expected_tid=%d tid=%d\n", expected_tid, tid);
